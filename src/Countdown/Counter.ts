...
class Counter extends Control {
  ...
  constructor(parentNode: HTMLElement, elName: string) {
    ...
    this.expiredDate = new Date("November 22, 2021 00:00:00").getTime();
  }
  ...
  countdown() {
    setInterval(() => {
      ...
      if (this.title.node.innerHTML === "day") {
        value = Day;
      } else if (this.title.node.innerHTML === "hour") {
        value = Hour;
      } else if (this.title.node.innerHTML === "minute") {
        value = Minute;
      } else if (this.title.node.innerHTML === "second") {
        value = Second;
      }
